...
export default function CirclesPage() {
  ...
  const contracts = [
    { name: "CircleVault", addr: CIRCLE_VAULT_ADDRESS, desc: "Group savings vault" },
    { name: "LendingPool", addr: LENDING_POOL_ADDRESS, desc: "Loan management" },
    { name: "CreditScore", addr: CREDIT_SCORE_ADDRESS, desc: "Credit reputation" },
    { name: "LoopToken", addr: LOOP_TOKEN_ADDRESS, desc: "LOOP rewards" },
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setShowContribute(true)} className="btn-glow text-sm">+ Contribute</button>
      </div>
      ...
      <div className="glass-card p-6">
        ...
        <div className="space-y-3">
          {contracts.map((c) => (
            <div key={c.name} className="flex items-center justify-between p-4 rounded-lg bg-[var(--bg-secondary)] border border-[var(--glass-border)]">
              <div>
                <div className="font-semibold text-sm">{c.name}</div>
                <div className="text-xs text-[var(--text-muted)]">{c.desc}</div>
              </div>
              <div className="flex items-center gap-3">
                <span className="font-mono text-xs text-[var(--text-secondary)]">{truncateAddress(c.addr, 6)}</span>
                {c.addr && c.addr !== "0x" && (
